...
def dados_proposta_lambda(lambdA:float,y:list,f:list,sigma:list,U:list,alpha_priori_lambda: float = 0.01, beta_priori_lambda:float = 0.1,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> tuple:
    ...
    U: list
        Lista com os valores da variavel de mistura U dimensão: M elementos
    ...
    q_d1 = (alpha_priori_lambda-1)/lambdA - beta_priori_lambda + np.sum(np.multiply(np.multiply(np.array(sigma*M, dtype=float).reshape(M,tau)**-1,np.repeat(np.array(list(U)).reshape(M,1),tau,axis=1)),np.multiply(np.matmul(f,matriz_lambda_d1),(y-np.matmul(f,matriz_lambda(lambdA,maturidade).T)))))

    q_d2 = -(alpha_priori_lambda -1)/lambdA**2 + np.sum(np.multiply(np.multiply(np.array(sigma*M, dtype=float).reshape(M,tau)**-1,np.repeat(np.array(list(U)).reshape(M,1),tau,axis=1)),np.multiply((y - np.matmul(f,matriz_lambda(lambdA,maturidade).T)),np.matmul(f,matriz_lambda_d2)) - np.multiply(np.matmul(f,matriz_lambda_d1),np.matmul(f,matriz_lambda_d1))))

    media_sd = (lambdA - (q_d1/q_d2),  np.sqrt(-q_d2**-1))

    return media_sd

def log_alvo_lambda(lambdA:float,y:list,f:list,sigma:list,U:list,alpha_priori_lambda: float = 0.01, beta_priori_lambda:float = 0.1,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> float :
    ...
    log_densidade = np.multiply(np.log(lambdA),(alpha_priori_lambda-1)) - lambdA*beta_priori_lambda - 0.5*np.sum(np.multiply(np.multiply(np.array(sigma*M, dtype=float).reshape(M,tau)**-1,np.repeat(np.array(list(U)).reshape(M,1),tau,axis=1)),(y - np.matmul(f,matriz_lambda(lambdA,maturidade).T))**2))
...
def densidade_grau_liberdade(log_nu:float,U:list,alpha_nu:float = 12.0,beta_nu:float = 0.8) -> float:
    '''Função para calcular o log da função de densidade do log de um determinado grau de liberdade

    log_nu: float
      Valor do log do grau de liberdade nu desejado para o cálculo da densidade
    U: list
      Lista com os valores da variavel de mistura U dimensão: M elementos
    alpha_nu: float
      Valor de alpha da priori de nu
    beta_nu: float
      Valor de beta da priori de nu
    return = log_d_nu: float
        Valor do log da densidade a posteriori de nu
    '''
    nu = np.exp(log_nu)
    log_d_nu = np.log(nu/2)*(len(U)*nu/2) + np.log(nu)*(alpha_nu-1) - 0.5*nu*(2*beta_nu + np.sum(U - np.log(U))) - np.log(gamma(nu/2))*len(U) + log_nu
    return log_d_nu

def gera_nu(it:int,U:list,nu_prev:float,llb:float,mu:float,s2:float,a0:float = 12.0,b0:float = 0.8) -> float:
    '''Função para gerar o próximo valor do grau de liberdade nu da cadeia do Metropolis-Hastings adaptativo

    it: int
      Valor da iteração da cadeia
    U: list
      Lista com os valores da variavel de mistura U dimensão: M elementos
    nu_prev: float
        Valor do estado anterior do grau de liberdade nu
    llb: float
        Valor do log da variável adaptativa lambda do algoritmo de MH adaptativo
    mu: 
        Valor da variável adaptativa média mu do algoritmo de MH adaptativo
    s2: 
        Valor da variável adaptativa covariancia s2 do algoritmo de MH adaptativo
    a0: float
      Valor de alpha da priori de nu
    b0: float
      Valor de beta da priori de nu
    ...
    ### passo 1
    lnu_prop = lnu_prev + np.random.normal(0,ni,1)
    while (lnu_prop<=np.log(2)) or (lnu_prop>=np.log(40)):
        lnu_prop = lnu_prev + np.random.normal(0,ni,1)
    ...
    ### passo 3
    a2 = densidade_grau_liberdade(lnu_prop,U,a0,b0)
    b2 = densidade_grau_liberdade(lnu_prev,U,a0,b0)
    alpha = min(1,np.exp(a2-b2))

    if (u2 < alpha):
        nu_ac = np.exp(lnu_prop)
    else:
        nu_ac = nu_prev
    
    lnu_ac = np.log(nu_ac)
    global nu_llbi
    nu_llbi = llb + gm * (alpha - 0.234)
    global nu_mui
    nu_mui = mu + gm * (lnu_ac - mu)
    global nu_s2i
    nu_s2i = s2 + gm * ((lnu_ac - mu)**2 - s2)
  
    return nu_ac

def MH_lambda(lambda_anterior:float,y:list,f:list,sigma:list,U:list,alpha_priori_lambda:float = 0.01, beta_priori_lambda:float = 0.1, maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> float:
  '''Função do algoritmo MCMC Metropolis Hastings para geração de lambda
    ...
    return = log_densidade: float
        Valor do log da densidade da distribuição alvo de lambda
    '''
  est_anterior = dados_proposta_lambda(lambda_anterior,y,f,sigma,U,alpha_priori_lambda, beta_priori_lambda, maturidade)
  proposta = np.random.normal(est_anterior[0],est_anterior[1],1)
  est_proposta = dados_proposta_lambda(proposta,y,f,sigma,U,alpha_priori_lambda, beta_priori_lambda, maturidade)

  alpha = min(1,np.exp( log_alvo_lambda(proposta,y,f,sigma,U,alpha_priori_lambda,beta_priori_lambda,maturidade) - log_alvo_lambda(lambda_anterior,y,f,sigma,U,alpha_priori_lambda,beta_priori_lambda,maturidade) + dnorm_log(x=lambda_anterior,mu=est_proposta[0], sd=est_proposta[1]) -  dnorm_log(x=proposta,mu=est_anterior[0], sd=est_anterior[1])))
...
def gen_sigma(y:list,f:list,lambdA:float,U:list,alpha_priori_sigma:float = 5,beta_priori_sigma:float = 0.2,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> list:
    ...
    U: list
        Lista com os valores da variável de mistura U em cada momento no tempo dimensão: M elementos
    ...
    soma = np.einsum('ij->j',np.multiply(np.repeat(np.array(U).reshape(y.shape[0],1),len(maturidade),axis=1), (y - np.matmul(f,matriz_lambdA.T))**2))
...
def gen_U(y:list,f:list,lambdA:float,sigma:list,nu:float,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> list:
    '''Função para geração dos valores da variável de mistura U no tempo
    ...
    nu: float
        Valor referente aos graus de liberdade nu da t-student
    maturidade: list, default [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]
        Maturidades escolhidas para realização do cálculo dimensão: tau elementos
    return = U: list
        Lista com os valores da variável de mistura geradas a partir de sua condicional a posteriori
    '''
    alpha = (nu + len(maturidade))/2
    beta = 0.5*(nu + np.einsum('ij->i',(np.repeat(np.array(sigma).reshape(1,len(maturidade)),y.shape[0],axis=0)**-1)*(y - np.matmul(np.delete(f,(0),axis=0),matriz_lambda(lambdA,maturidade).T))**2))
    U = list(np.concatenate([np.random.gamma(shape=alpha, scale=1/x, size=1) for x in beta]))
...
def y_t(ft:list,lambdA:float,sigma:list,Ut:float,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> list:
    ...
    sigma: list
        Lista da diagonal da matriz de covariância das taxas sigma dimensão: tau elementos
    Ut: list
        Valor da variável de mistura U no tempo t
    maturidade: list, default [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]
        Maturidades escolhidas para realização do cálculo dimensão: tau elementos
    return = novo_f: list
        Lista com os valores dos fatores latentes no tempo t
    '''
    novo_y = np.matmul(matriz_lambda(lambdA,maturidade), ft.T) + Ut**(-0.5)*np.random.multivariate_normal([0]*len(maturidade), np.diag(sigma) , 1).reshape(1,3)
    return novo_y

def FiltroKalman(y:list,A:list,W:list,sigma:list,mu:list,U:list,lambdA:float,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]) -> tuple:
    ...
    Qt = (np.matmul(Ft,np.matmul(Rt[0,:,:],Ft.T)) + np.diag(sigma)/U[0]).reshape(1,y.shape[1],y.shape[1])
    ...
    for it in range(1,y.shape[0]):
        ...
        Qt = np.append(Qt,(np.matmul(Ft, np.matmul(Rt[it,:,:], Ft.T)) + np.diag(sigma)/U[it]).reshape(1,y.shape[1],y.shape[1]),axis=0)
...
def FFBS(y:list,A:list,W:list,sigma:list,mu:list,U:list,lambdA:float,maturidade:list = [3,6,9,12,15,18,21,24,30,36,48,60,72,84,96,108,120]):
    ...
    filtro = FiltroKalman(y,A,W,sigma,mu,U,lambdA,maturidade)
...
nu_gibbs = 20.0
U_gibbs = list(np.concatenate([np.random.gamma(shape=nu_gibbs/2, scale=2/nu_gibbs, size=1) for x in range(y.shape[0])]))

nu_mui = 0 
nu_llbi = np.log(2.38**2) 
nu_s2i = 1 
...
for it in range(10**5+1):
    ...
    ffbs = FFBS(y,A_gibbs,W_gibbs,sigma_gibbs,mu_gibbs,U_gibbs,lambda_gibbs,maturidade)
    lambda_gibbs = MH_lambda(lambda_gibbs,y,ffbs,sigma_gibbs,U_gibbs,alpha_priori_lambda, beta_priori_lambda, maturidade)
    mu_gibbs = gen_mu(ffbs,A_gibbs,W_gibbs,mu_priori,S_priori)
    sigma_gibbs = gen_sigma(y,ffbs,lambda_gibbs,U_gibbs,alpha_priori_sigma,beta_priori_sigma,maturidade)
    ...
    nu_gibbs = gera_nu(it,U_gibbs,nu_gibbs,nu_llbi,nu_mui,nu_s2i,a0,b0)
    U_gibbs = gen_U(y,ffbs,lambda_gibbs,sigma_gibbs,nu_gibbs,maturidade)
    ...
    if (it > 20000) and (it%16 == 0):
        ...
        open(diretorio + '\\nu.txt', 'a').write(" ".join(map(str, np.array([nu_gibbs])))+'\n')
        open(diretorio + '\\U.txt', 'a').write(" ".join(map(str, U_gibbs))+'\n')
